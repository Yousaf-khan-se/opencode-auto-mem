const VALID_TARGETS = ["memory", "identity", "user", "project", "corrections", "environment"];
const VALID_ACTIONS = [
  "index",
  "read",
  "write",
...
export function validateHeadingPath(headingPath: unknown): string[] {
  if (!Array.isArray(headingPath) || headingPath.length === 0) {
    throw new Error(
      "headingPath must be a non-empty array of heading titles, e.g. [\"Environment Memory\", \"Commands\"]"
    );
  }
  ...
  if (path.length === 0) {
    throw new Error(
      "headingPath must contain at least one non-empty heading title"
    );
  }
  return path;
}
